...
- Actions collapse to a trailing action column: `⚙` and `−`, plus `↑ ↓` where order is content. No wrapped second line.
  - **Amended 2026-08-06, owner decision, from browser-verifying this slice.** On a **table row**, `↑ ↓` move into the row's `⚙` sheet and only `⚙ −` stay on the row. Measured at a 390px viewport: four 44px targets (spec 12 §1's floor, which does not shrink) made `.unit-row-actions` a fixed **196px at every viewport** — 55% of the 358px table — leaving Term 74px and Definition 88px, against the **274px** the same column gets in learner mode. The text no longer clipped, but it wrapped every ten characters, so §1's own opening promise and §14's "recognisably the same page" both failed on exactly the screen this slice owns. Dropping to two icons takes the rail to 100px and gives Definition 135px. **Cards keep `↑ ↓` inline** (Examples, Exercises): they are full-width and have no column to squeeze. One consequence worth knowing: a read-only lexicon row is now offered a `⚙` it previously had none of, because reordering lives inside it and reordering writes `unit.itemIds`, which the Book owns — the sheet's _fields_ guard on `canEditRow`, not the sheet itself.
