# Plan 0025: Generated exercises

Status: **drafted** · Owner: Moe · Date: 2026-08-17 · Prerequisite: [plan 0024](0024-exercise-difficulty-ladder.md) (generation is only principled once exercises are ranked) · Origin: owner proposal, 2026-08-17 — "remove all standard exercise questions and just let them be generated based on the content; specific task creation should still be possible"
...
This plan derives the index from the item set and 0024's ladder, and keeps authoring as an override. It is the third thing in this codebase to become derived rather than stored, after progress and the symmetric link closure.
...
- **No generated _content_.** Sentences, glosses, examples and cloze markup are authoring. This plan only decides which exercises to build over items that already exist. (Unchanged from 0024 §10.)
- **No learner state in generation.** The generator is pure over content, like `buildUnitSession` — same content, same task set, on every device. Adaptation lives in 0024 §5's review climb, which reads the rung.
...
Pure function over content: unit items (kinds, asset refs, cloze markup) + the domain lexicon + 0024's rank table + optional targets (§5) + a question budget → a task set. No `Rng`; shuffling stays where it is, in session building.
...
Of these, the two with actual evidence behind them are 0024's comprehension-before-production ordering and spacing; the rest are defensible design heuristics and are labelled as such rather than dressed up as findings.
...
**Adopted — how far up the ladder this item should be taken.** "Passive vocabulary" versus "active vocabulary" is a real teaching decision every course makes, and one no amount of learner data can infer, because it is about intent. It maps exactly onto 0024's bands: recognise-only / produce / master. Default is the full ladder for the item's kind, so most items never set it.
...
1. **0024 §8's per-item band coverage is promoted to required.** An author has to be able to see that a unit tops out at band A, or the silence is the whole experience.
...
**Ride 0023's bump if it lands first.** Plan 0023 already bumps 1 → 2 for the `components` reshape. Two bumps cost two rollout waits; one costs one.
...
2. **Supplement mode**, opt-in per Book — unit sessions become authored ∪ generated-for-missing-types, ordered by 0024's bands. The opt-in flag is what keeps this from silently lengthening every existing unit's session on the day it ships.
...
5. **Does 0024 §10 survive?** Most of its wizard dissolves into §8's preview. The part that does not is the cloze-blank suggester — but that authors *item* markup, not tasks, so it may belong with the editor work rather than with either plan.
