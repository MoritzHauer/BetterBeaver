...
Neither is a prerequisite, and this plan does not conflict with either — but two seams are worth naming before anything is built.

**0026 (generated exercises) fits this rather than fights it.** Its thesis is that a `Task` carries no authored decision because the decision already lives on the item — `cloze`'s blank is markup in the sentence, `minimal-pair`'s pair *is* the item. A `question` item is the same shape: its options, their correctness and its two labels are all payload, and its task type is derivable (`labels` present → `assign`, absent → `choice`). So a `question` item is generatable by 0026's generator on the same terms as the eleven existing types, and needs no special case. What it must **not** do is generate authored questions from nothing — that is 0026's own "no generated _content_" non-goal, and it holds here unchanged.

**The one real seam is exam references.** §3 has an exam list `taskId`s, and 0026 makes some tasks generated rather than authored. A generated task has no stable authored id, so an exam must only ever reference an **authored** task — which 0026 keeps first-class forever ("authored tasks stay first-class; generation fills what a unit did not author"). Whichever plan lands second owns the validator rule that says so. If 0026 lands first, this plan adds it; if this lands first, 0026's generator must not be able to satisfy an exam reference.

**0025 (progression engine) touches this only through its level table.** A `choice`/`assign` question would need a rung like every other exercise type. This plan deliberately does not assign one — the ladder is 0025's to define, and guessing a rung here would put a second source of truth next to it.
