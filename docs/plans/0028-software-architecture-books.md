...
**Working tree:** `content.local/<book>/`, checked out and refreshed by `scripts/content-checkout.sh` (2026-09-18; it supersedes the hand-managed `scratch.local/softwarearchitektur/` this plan started with, and carries its own git history so a re-pull replays local edits onto whatever was published meanwhile). Authoring material that is not content — an exam's explanations, a lesson mapping — sits in `content.local/authoring/`, outside any checkout directory, so no republish can see it. Git-ignored by the repo's `*.local` rule, it is the Book's working copy across runs. It is never the repo's `content/` (plan 0015 decision 10), and it is never committed. The backend's `versions` table is its history, and every republish is its backup.
...
- **B2 authored 2026-09-18 — the iSAQB Beispielprüfung, 38 questions / 52 points, `sa-exam-isaqb`. Not yet republished** (owner call pending). Transcribed by parser, not by hand: `scripts/exam-from-markdown.py` reads the vault paper and asserts every parsed answer against the paper's own Lösungsschlüssel table before writing anything — 38 questions, 52 points, zero disagreements — which is what makes "verbatim, including the source's label order" (B2) true by construction rather than by care. A-Fragen and P-Fragen become `choice`, K-Fragen become `assign` with the source's own category pair as `labels` (affirmative first in every one of the 13). Each question is its own exam-only task in no unit (class (ah)); the source's italic instruction line becomes the task's `instructions`. The hand-written half — `explanation` per question, `why` on every option a learner can plausibly mis-mark, and the question→lesson mapping — lives in `content.local/authoring/isaqb-exam.json` with `explanationGenerated: true` on all 38. **Deviation from B2's wording, deliberate:** B2 says "a `why` per option"; whys sit on the misleading options only, per [content-practices](../content-practices.md) §5, since a rationale under an obviously-correct option is padding the learner reads past. **Only 6 of 38 carry a `lessonId`** (the L1 questions): a dangling one is a validation error (class (ag)), so the mapping is authored in full for all ten lessons and emitted as each lesson lands — the other 32 bucket as "Other" in the readiness report until phase A catches up, and a re-run of the generator is all that lights them up. Verified: validator green; `overlap-check` **exit 0 over our own words**, and the one hit over the whole tree is inside a verbatim iSAQB option that the textbook happens to phrase the same way (recorded in content-practices §7); `pnpm check` green at 1002 tests; **31/31 browser pass** on the packed `.bbbook` in a fresh profile (`scratch.local/verify-sa-exam.mjs`) — the exam card reads "38 Fragen · 52 Punkte · 75 Minuten" after the lessons with no KI-generiert badge, the intro carries the licence condition in full, an A-Frage's cap refuses the extra pick instead of replacing the answer, nothing grades before Abgeben, and the report's arithmetic matches §4 by hand (one 1-point question right, 37 blank → 1 von 52 · 1,9 % · nicht bestanden; Grundlagen 1/6 · 16,7 %, Other 0/46), while the run wrote `bb.exam.sa-exam-isaqb` and touched neither `bb.item.*` nor `bb.streak`. **Found on the way and fixed in the repo:** `packages/schema/src/content.test.ts` never read `exams/` and never passed `exams` to `validateContent`, so the on-disk validator — content-practices §7 step 1, the gate every authoring run leans on — reported any exam-bearing Book as a dangling `topic.examIds` plus one orphaned task per question. Same shape as the `createDocumentContentSource` gap 0027's browser pass caught, on the other path.
- **Three exam-UI fixes, 2026-09-18, all found by playing the real exam** (owner-reported; `apps/web`, outside 0027's own scope — it was built and verified against a **two**-question synthetic exam, which is why none of them showed up then). (1) The intro's four buttons were bare siblings of `<main>` with no wrapper, so they had no gap at all; they now sit in the `.exam-actions` the runner already used. (2) The question navigator was a wrapping grid — at 38 questions, six rows of cells above the fold, pushing the question itself off screen. It is now one horizontally scrolling row beside the clock (`.exam-header`), with the current cell scrolled into view on every move, since the bar holds about six cells at phone width. (3) `.prompt` is a flat 1.5rem everywhere, which suits a Kyrgyz headword and not a 236-character German exam stem. `choice`/`assign` stems now scale by their own **character count** — handed to CSS as `--stem-len` by a new `QuestionStem`, sized by one `clamp()`. Length rather than measured height on purpose: no ResizeObserver, no measure-then-reflow loop, and the same question is the same size on every device; the clamp's floor (1.1rem, still above body text) is what covers the approximation. Q4, the longest stem in the paper, went from filling the screen before its first option to 23 % of viewport height with all three rows and the buttons visible. `pnpm check` green at 1002 tests, and the B2 browser pass re-run at 31/31 after all three.
- **Exam navigation, 2026-09-18** (owner-requested, `apps/web`): the intro's Back moved out of the action row into a `.screen-header` labelled with the Book title, the way every other screen does it; the runner gained its own header Back, which returns to the intro rather than out of the Book — leaving mid-run was always safe (the deadline is wall-clock, so the attempt keeps running), what was missing was a way out that looked like the rest of the app. And the intro now offers **Weitermachen / Neu starten** on a live attempt instead of Resume alone: before this a half-finished run could only be sat out or submitted, since nothing could discard it. Restart is behind a `ConfirmSheet` because it throws the answers away; `startAttempt` overwrites the record's attempt outright, so the confirm's Yes is both the discard and the restart. Verified 12/12 in a fresh profile (header Back lands on the Book, runner Back returns to a still-running attempt, cancelling the restart keeps the answers, confirming clears them and reopens at question 1), `pnpm check` green at 1002 tests, B2 pass still 31/31.
- **Four owner calls, 2026-09-18** (content + `apps/web`), all from playing the shipped Book. (1) **Descriptions are one sentence.** The exam's now reads as a single line naming source and copyright holder, which is what the licence actually requires; the redistribution and no-real-exam conditions stay in the exam's `resource` title and in this plan rather than on screen. The Book description absorbs the attribution by **rewriting** its third sentence, not by gaining a fourth — appending the full licence paragraph there was tried first and turned the My Books card into a wall of text, which is the thing the call was against. This also retires 0027 §2's `.exam-description` `pre-line` rule, dead the moment descriptions stopped having paragraphs. **Note for §2**: attribution is now carried by one learner-visible site in full and one in brief, not two in full. (2) **Info units are legal.** Validator class (i) gains its one exemption: a unit with no tasks **and no items** but at least one note. Items without tasks stay an error — that is the authoring mistake the class exists to catch — and a unit with none of the three is a third, still-wrong thing. The app already rendered such units (`UnitScreen.check.test.tsx`'s `notesOnlyUnit` fixture predates this); only the validator forbade authoring one. Its last page's bar now reads **Done** rather than "Practice 0": Practice still has to run, because its summary is the only way to the next unit, but offering it by the one thing it does not do was wrong. (3) **L0.1 is that unit.** The exam-rules unit is notes-only now — 7 tasks and its 3 authored questions deleted, its 11 concepts left in the `sa` lexicon so the note's tap-to-lookup still resolves them. The Book is 50 items / 65 tasks, down from 53 / 72. (4) **The Book card gained a footer row** under the progress bar: count left, the `private` tag centred, the ⋯ menu right, as a three-column grid so the tag sits in the card's middle rather than wherever the count's width leaves it. The open menu is absolutely positioned — a `<details>` opening inside the last grid column was squeezed to that column's width, which the browser pass caught. Verified: validator green, `overlap-check` exit 0 over our words, `pnpm check` 1002 tests, exam pass 33/33 and a new 13/13 pass for the footer geometry and the info unit.
- **Card text and the footer, 2026-09-18** (owner, from the running app). Descriptions shrank again — the exam's to one short clause (75 chars; that the explanations are ours is already a per-question `explanationGenerated` badge in the report, so the description need not say it), and the Book's reordered so the sentence saying **what the Book is** comes first and plan 0028 §2's two affiliation disclaimers follow it. That order matters because the card now **clamps its description to two lines** (`.card-description`, My Books and Library alike) — the cap the owner asked for, applied to the box rather than to the content: a character limit would have to be either long enough for §2's mandatory disclaimers or a warning that fires on this Book for ever, and the full text is never lost since `BookScreen` renders it whole. The footer row was rebuilt after the first attempt looked sparse: measured at 72px tall with its content floating 32px under the progress bar, because `.card > div`'s 1rem inset outranks a bare `.card-footer` and the `⋯` summary carried 40px of padding. Now 32px, selected as `.card > .card-footer` so its own padding wins, with the tag as a pill rather than stray text between two controls. Two bugs the browser pass found rather than reasoning: a `<details>` opening inside the last grid column was squeezed to that column's width, and on a card with **no** tag the menu was auto-placed into the middle column and sat centred — it is pinned to `grid-column: 3` now. Verified 15/15 on the card and info-unit pass, 32/32 on the exam pass, `pnpm check` 1002 tests, overlap-check exit 0.
- **One way in, 2026-09-18** (owner; `apps/web`, a net deletion). The Book screen's Daily Review, Practice and Vocabulary cards are gone, and so are My Books' Vocabulary and Daily Review chips — Play alone remains, since it already routes to Daily Review whenever anything is due. The due count rides on **Continue learning** instead of owning a card: the review icon with a small number at the button's bottom-right corner. Re-practising is reached from the lesson or the unit that holds it, which is where `lessonPracticeTargets` still lives. **The Vocabulary screen is deleted outright** (755 lines), with its ad-hoc study session, both routes (`/domains/<id>/vocab`, `/study`), `AdhocSession` in `App.tsx` and a now-unreferenced `content/links.ts` going with it; `content.test.ts`'s companions — the lexicon itself, `EntryPopup`, tap-to-lookup and the word/entry stores — are untouched, because deleting those would have taken lookup with them and nobody asked for that. What the sweep turned up is worth recording: every other mention of `VocabularyScreen` in the repo was a **comment**, so the file had exactly one importer. Card text is capped at two lines on lesson and unit cards as well now, the same `.card-description` the Book cards use. Verified: `pnpm check` 1000 tests (down two with the retired route cases), and four browser passes — 32/32 exam, 15/15 card and info unit, 13/13 navigation trim, 6/6 due chip (absent with nothing due; icon plus count at 12px/8px from the button's corner with two overdue items; Continue learning then lands on Daily Review). Two tests changed rather than broke: `PreviewDiff` pinned "hides Play and Daily Review and **keeps Practice**", whose subject no longer exists, and `BookLesson.edit` asserted all four cards render in learner mode — both now assert the cards are gone.
