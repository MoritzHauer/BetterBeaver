...
5. **The same mismatch already exists for an authored cloze task in unit practice, and was left alone.** `startDrill` is handed `unit.itemIds`, while an authored cloze question grades `<itemId>::c1`; `advanceDrill` then consumes the visit without paying off the count. Confirmed with a throwaway test against `advanceDrill` directly. It predates this plan, it changes behaviour for Books that have *not* opted in, and phase 1 promises not to — so it is recorded in STATUS's backlog instead of fixed here.
...
- **"Phase 1: ... no content edited."** Held: no file under `content/` changed, `CONTENT_SCHEMA_VERSION` did not move, and the demo Book does not opt in. Nothing generated reaches a learner until an author ticks the switch.

Not browser-verified in the app itself — the coverage grid was rendered and screenshotted in both themes against the real stylesheet (which is what caught a `display: block` on a `td` collapsing the grid into one column, and the ten rungs not fitting a 400px screen), and its behaviour is covered by `UnitScreen.exercises.test.tsx`. An `apps/web:verify` pass through the editor is still worth doing.
